...
category: 'projects'
tags: ['troubleshooting', 'nextjs', 'blog']
---
...
클라이언트에서 `{theme}`에 들어가는 텍스트 노드가 "system"인데 서버에서는 다른 값이 들어가고 있나보다.
...
[공식문서](https://github.com/pacocoursey/next-themes?tab=readme-ov-file#avoid-hydration-mismatch)대로 클라이언트에서 페이지가 마운트된 이후에만 ThemeSwitch를 렌더링하는 방법이다.
...
const ThemeSwitch = () => {
  ...
  // false false -> true true(시스템 설정이 dark인 경우)
  console.log(hasTheme, isDarkTheme)
...
서버 & 페이지 마운트 이전에는 resolvedTheme === undefined이다.

커스텀 훅에 `hasTheme`을 만들어 resolvedTheme이 undefined이면 아무것도 렌더링하지 않게 만들었다.

그리고 페이지 마운트 후에 테마값이 제대로 들어가면 다크모드인지 아닌지에 따라 아이콘을 조건부 렌더링한다.

<hr />

hydration에 대한 자세한 내용은 [여기로](/posts/fe/react/hydration)
